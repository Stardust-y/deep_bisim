...
from src_HER_level3.insulin_causal_x4_x8_simpler import CausalGraph
import torch
import torch.nn.functional as F
from src_HER_level3.env_do_all_2reward import PreCBN
        ...
        if len(list_last_vertex) > 0:
            ...
            for idx in range(len(self.reward_goal) - 1, -1, -1):  # 逆序遍历
                if idx == len(self.reward_goal) - 1:  # 最后一个
                    ...
                    goal_temp = []
                    if 4 in self.reward_env[idx].envs[0].vertex:
                        goal_temp.append([self.reward_env[idx].envs[0].state.get_value(4) - 10.0,
                                          self.reward_env[idx].envs[0].state.get_value(4) + 10.0])
                    elif 6 in self.reward_env[idx].envs[0].vertex:
                        goal_temp.append([self.reward_env[idx].envs[0].state.get_value(6) - 10.0,
                                          self.reward_env[idx].envs[0].state.get_value(6) + 10.0])
                    else:
                        for item in self.reward_env[idx].envs[0].vertex:  # 存储goal，下个阶段的model需要使用
                            goal_temp.append([self.reward_env[idx].envs[0].state.get_value(item) - 10.0,
                                              self.reward_env[idx].envs[0].state.get_value(item) + 10.0])
